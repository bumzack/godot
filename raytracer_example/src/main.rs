...
fn run_cuda_stuff(w: usize, h: usize, size_factor: f32, anitaliasing: bool, antialiasing_size: usize) {
    ...
    run_cpu_chapter14_with_aa(&backend_cpu, "cuda", w, h);
    run_cpu_compare_to_cuda(&backend_cpu, "cuda", w, h);
    run_cpu_shadow_glamour_shot(&backend_cpu, "cuda", size_factor, anitaliasing, antialiasing_size);
    run_cpu_soft_shadow(&backend_cpu, "cuda", size_factor, anitaliasing, antialiasing_size);
}
...
fn run_cpu_shadow_glamour_shot(b: &dyn Backend, backend_name: &str, size_factor: f32, antialiasing: bool, antialiasing_size: usize) {
    let filename_cpu_single = format!("{}_shadow_glamour_shotcpu_single_{:2}x{}x{}.png", backend_name,size_factor, antialiasing, antialiasing_size);
...
fn run_cpu_soft_shadow(b: &dyn Backend, backend_name: &str, size_factor: f32, antialiasing: bool, antialiasing_size: usize) {
    let filename_cpu_single = format!("{}_test_soft_shadow_cpu_single_{:2}x{}x{}.png", backend_name,size_factor, antialiasing, antialiasing_size);
