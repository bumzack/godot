...
use crate::micrograd_rs_v3::OpEnumV3;
use crate::micrograd_rs_v3::ValueRefV3;

pub fn draw_graph(root: ValueRefV3, filename: String) {
...
pub fn create_petgraph(root: ValueRefV3) -> Graph<String, String> {
...
fn add_nodes_petgraph(graph: &mut Graph<String, String>, node: &ValueRefV3, parent_index: NodeIndex, op: &OpEnumV3) {
    let p_idx = match op {
        OpEnumV3::NONE => parent_index,
        // all operations ...
        _ => {
...
fn format_value_v2(root: &ValueRefV3) -> String {
    format!(
        "{} | data {:.4} | grad {:.4} ",
        root.borrow().label(),
        root.borrow().data(),
